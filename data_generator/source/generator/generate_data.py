...
def update_input_config(current_level=1):
    try:
        ...
        #    last_number_of_dups=fd_config.getint("level"+str(current_level-1),"number_of_dup_values")
        #    fd_config["level"+str(current_level)]["number_of_distinct_vals"]=str(int(total_vals/last_number_of_dups))
        #
        
        number_of_distinct_vals=fd_config.getint("level"+str(current_level),"number_of_distinct_vals")
        
        #is_parent=bool(fd_config["level"+str(current_level)]["is_parent"])
        #total_vals=fd_config["level"+str(current_level)]["total_vals"]
        #number_of_distinct_vals=fd_config["level"+str(current_level)]["number_of_distinct_vals"]
        
        #if is_parent==True:
        #    print(str(number_of_distinct_vals))
        
        #if current_level==1:
        #    fd_config["level"+str(current_level)]["number_of_dup_values"]=str(number_of_distinct_vals)
        #else:
        #    last_number_of_dups=fd_config.getint("level"+str(current_level-1),"number_of_dup_values")
        #    fd_config["level"+str(current_level)]["number_of_dup_values"]=str(number_of_distinct_vals*last_number_of_dups)
        ...
        #    fd_config["level"+str(current_level)]["total_vals"]=str(v_distinct_values)
        elif is_parent==True:
            #print("this level value:"+number_of_distinct_vals)
            ...
            if level==first_level:
                if is_parent==False:
                    ...
                    for i in range(1,num_of_distinct_vals+1):
                        ...
                        #if max_tuples < v_output_file_index:
                        #    return
                ...
                if is_parent==False:
                    ...
                    for i in range(1,num_of_distinct_vals+1):
                        ...
                        if i>number_of_dup_values:
                            ...
                            #start_point=v_addup+(i-1)*next_num_of_distinct_vals+1
                            #end_point=v_addup+i*next_num_of_distinct_vals+1
                    ...
                    for i in range(start,end):
                        ...
                        if level==first_level-1:
                            ...
                            #print(number_of_dup_values-diff)
                        ...
                        not_dup_factor=parent_index-1
                        #if to_be_duplicated_cnt>number_of_dup_values:
                        if i>number_of_dup_values:
                            #if level==first_level-1:
                            #    print("this is ::::"+str(next_num_of_distinct_vals))
                            #f_part,i_part=math.modf(i/number_of_dup_values)
                            #start_point=int(i_part)*to_be_duplicated_cnt*next_num_of_distinct_vals-number_of_dup_values
                            #end_point=int(i_part)*to_be_duplicated_cnt*next_num_of_distinct_vals-number_of_dup_values+1
                            start_point=(number_of_dup_values)*next_num_of_distinct_vals+(i-number_of_dup_values)
                            end_point=(number_of_dup_values)*next_num_of_distinct_vals+(i-number_of_dup_values)+1
...
def log_calls(function_name,start_time,type=""):
    if type!="new_run":
        if start_time is None:
            ...
            return v_start_time
        #rootLogger.info("started at:"+dt_now)
        else:
            dt_now = datetime.now().strftime(dt_format)
...
def fill_difference():
    try:
        ...
        #expected_total_tuples=fd_config.getint("level"+str(first_level),"total_vals")
        #total_distinct=fd_config.getint("level"+str(first_level-1),"number_of_dup_values")
        ##second_distinct=fd_config.getint("level"+str(first_level-1),"number_of_distinct_vals")
        #last_distinct=fd_config.getint("level"+str(first_level),"number_of_distinct_vals")
        #diff_distinct = expected_total_tuples/total_distinct - last_distinct
        ##total_third_distinct=fd_config.getint("level"+str(first_level-2),"number_of_dup_values")
        #if diff_distinct>0:
        #    #diff_test=(second_distinct*diff_distinct)/last_distinct
        #    #print(str(diff_distinct*total_distinct))
        #    fd_config["level"+str(first_level)]["total_vals_diff"]=str(int(diff_distinct*total_distinct))
        #
        #print(str(fd_config.getint("level"+str(first_level),"total_vals_diff")))
    except Exception as e:
...
def run_generation(number_of_datasets=1,output_path=".\\output\\"):
    try:
        ...
        for i in range(1,number_of_datasets+1):
            ...
            #max_tuples=fd_config.getint("level"+str(first_level),"number_of_dup_values")
